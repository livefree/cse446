...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    # f(x,y)_x : A x + A^T x + B^T y
    tik = time.time()
    term1 = vanilla_matmul(A, x)
    term2 = vanilla_matmul(vanilla_transpose(A), x)
    term3 = vanilla_matmul(vanilla_transpose(B), y)
    toc = time.time()
    gradient: Vector = np.array(term1) + np.array(term2) + np.array(term3)
    
    print("\nvanilla solution run time = ", toc-tik)
    return gradient
...
def numpy_solution(
    ...
    tik = time.time()
    term1 = np.matmul(A, x)
    term2 = np.matmul(np.transpose(A), x)
    term3 = np.matmul(np.transpose(B), y)
    gradient: Vector = term1 + term2 + term3
    toc = time.time()
    print("\nnumpy solution run time = ", toc-tik)
    return gradient
